...
<body>
  ...
  <div class="logo-details">
    ...
    <ul class="nav-list" id="nav-list">
     ...
     <li>
       <a class="nav-link active " href="../datamhsSkripsi/mhsskripsi.php">
  ...
  <section class="home-section">
    <div class="container-fluid">
      ...
      <?php
          $ambildatasudah = mysqli_query($conn, "SELECT * FROM tb_mhs m, tb_skripsi p WHERE  m.nim = p.nim AND status_skripsi = 'LULUS'");
          $sudah = mysqli_num_rows($ambildatasudah);

          $ambildatabelum = mysqli_query($conn, "SELECT * FROM tb_mhs m, tb_skripsi p WHERE  m.nim = p.nim AND status_skripsi = 'BELUM MENGAMBIL'");
          $belum = mysqli_num_rows($ambildatabelum);

          $ambildatasedang = mysqli_query($conn, "SELECT * FROM tb_mhs m, tb_skripsi p WHERE  m.nim = p.nim AND status_skripsi = 'SEDANG MENGAMBIL'");
          $sedang = mysqli_num_rows($ambildatasedang);
      ?>
        ...
        <div class="col">
          <a href="mhssedangskripsi.php">
          <div class="card rounded-4 card-active   ">
            <div class="card-body">
              <p class="text-center">Jumlah Mahasiswa Sedang Skripsi</p>
      ...
      <br>
      <div class="h5 mt-4 mb-4 w-100">Mahasiswa Sedang Skripsi</div>
      ...
      <table id="example" class="table   rounded-3" style="width:100%">
        ...
        <tbody>
          <?php
          $ambildata = mysqli_query($conn, 'SELECT * FROM tb_mhs m, tb_skripsi p WHERE  m.nim = p.nim AND p.status_skripsi = "SEDANG MENGAMBIL"'  );
          $i = 1;
...
keyword.addEventListener('change', function(){
    ...
    //eksekusi ajax
    xhr.open('GET', 'getmhssedangskripsi.php?keyword=' + keyword.value, true);
    xhr.send();
});

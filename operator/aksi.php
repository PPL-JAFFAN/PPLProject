...
if (isset($_POST['add_dosen'])) {
   ...
   if(empty($nip)){
      ...
      $error_nip = "NIP harus 18 angka";
      $valid = false;
   }

   $cek = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM tb_dosen WHERE nip='$nip'"));
    if ($cek > 0){
      $error_nip = "NIP sudah ada";
      $valid = false;
    }
...
//delete all dosen
if (isset($_POST['delete_all_dosen'])) {

   $querydelete = mysqli_query($conn, "DELETE FROM tb_dosen");
   $querydeleteuser = mysqli_query($conn, "DELETE FROM tb_user WHERE status='dosen'");
...
if (isset($_POST['delete_all_mhs'])) {

   $querydelete = mysqli_query($conn, "DELETE FROM tb_mhs");
   $querydeleteuser = mysqli_query($conn, "DELETE FROM tb_user WHERE status='mhs'");
   $deletepkl = mysqli_query($conn, "DELETE FROM tb_pkl");
   $deleteskripsi = mysqli_query($conn, "DELETE FROM tb_skripsi");
...
if (isset($_POST['add_mhs'])) {
   ...
   $cek = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM tb_mhs WHERE nim='$nim'"));
    if ($cek > 0){
   ...
   if($valid == true){
      ...
      $addtopkl = mysqli_query($conn, "INSERT INTO tb_pkl (nim,status_pkl) VALUES('$nim', '$status_pkl')");
      $addtoskripsi = mysqli_query($conn, "INSERT INTO tb_skripsi (nim,status_skripsi) VALUES('$nim', '$status_skripsi')");

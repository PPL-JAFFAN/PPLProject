...
<body>
  ...
  <section class="home-section">
    <div class="container-fluid">
      ...
      <a class="btn btn-primary dropdown-toggle" href="2021.php" role="button" data-bs-toggle="dropdown" aria-expanded="false">
        2021
      </a>
      ...
      <div class="row row-cols-1 row-cols-md-3 g-4 mt-1">
          ...
          <?php
          $ambildata = mysqli_query($conn, "SELECT * FROM tb_mhs WHERE status='Aktif' AND angkatan = '2021'");
          $aktif = 0;
          while ($data = mysqli_fetch_array($ambildata)) {
              $aktif++;
              }

          $ambildata = mysqli_query($conn, "SELECT * FROM tb_mhs WHERE status='Nonaktif' AND angkatan = '2021'");
          $non_aktif = 0;
          while ($data = mysqli_fetch_array($ambildata)) {
              $non_aktif++;
              }

          $ambildata = mysqli_query($conn, "SELECT * FROM tb_mhs WHERE status='Cuti' AND angkatan = '2021'");
          $cuti = 0;
          while ($data = mysqli_fetch_array($ambildata)) {
              $cuti++;
              }
      ...
      <?php
      $datalulus = mysqli_query($conn, "SELECT * FROM tb_mhs WHERE status='Lulus' AND angkatan = '2021'");
      $lulus = mysqli_num_rows($datalulus);


      $dataud = mysqli_query($conn, "SELECT * FROM tb_mhs WHERE status='Undur Diri' AND angkatan = '2021'");
      $ud = mysqli_num_rows($dataud);

      $datado = mysqli_query($conn, "SELECT * FROM tb_mhs WHERE status='DO' AND angkatan = '2021'");
      $do = mysqli_num_rows($datado);

      ?>
      <div class="col">
      <a href="../lulus/2021.php">
      <div class="card rounded-4 ">
        <div class="card-body">
      ...
      <table id="example" class="table  bg-light rounded-3" style="width:100%">
        ...
        <tbody>
          <?php
          $ambildata = mysqli_query($conn, 'SELECT * FROM tb_mhs WHERE status = "Aktif" AND angkatan ="2021" ORDER BY  nama ASC, semester ASC'  );
          $i = 1;
          while ($data = mysqli_fetch_array($ambildata)) {
              $nim = $data['nim'];
              $nama = $data['nama'];
